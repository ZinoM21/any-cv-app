...
type Props = {
  ...
  /**
   * A customer value that can be used to differentiate widgets under the same sitekey in analytics
   * and which is returned upon validation. This can only contain up to 32 alphanumeric characters
   * including _ and -.
   * @see https://developers.cloudflare.com/turnstile/get-started/client-side-rendering/#configurations
   **/
  action?: string;
...
export default function TurnstileInput(props: Props) {
  useEffect(() => {
    ...
    const names = Object.keys(obj).reduce(
      (acc, name) => {
  ...
  return (
    <div
      ...
      data-before-interactive-callback={
        props.onBeforeInteractive ? functionNames.beforeInteractive : undefined
      }
      data-after-interactive-callback={
        props.onAfterInteractive ? functionNames.afterInteractive : undefined
      }
      data-unsupported-callback={
        props.onUnsupported ? functionNames.unsupported : undefined
      }
    ></div>
  );
}
